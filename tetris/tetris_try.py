...
class Tetris:
    ...
    def update(self):
        ...
        if self.game_state == "running":
            ...
            if pyxel.frame_count % FALL_SPEED == 0:
                self.block.block_falling(self.block.get_block_sections(self.starting_position), self.board.board)

            if pyxel.btnp(pyxel.KEY_DOWN, 10, 2):
                
                if pyxel.btnp(pyxel.KEY_DOWN and pyxel.KEY_RIGHT, 10, 2):
                    self.direction = DOWN_RIGHT
                    self.block.move_block(self.direction, self.starting_position, self.board.board)
    ...
    def draw(self):
        ...
        Text.show_text(self)
        self.board.draw_block(self.starting_position)
        self.board.draw_next_block()
        # self.board.add_block_to_board(self.starting_position)
